...
struct Method : public Tracer {
    Method();
    Method(SlotMethod slot_method);
    Method(UserMethod user_method);

    std::variant<SlotMethod, UserMethod> method;
...
public:
    ...
    virtual OrobaObject* PrimitiveMessage(std::string name, std::vector<OrobaObject*> args, LocalCollector& collector) = 0;
