...
SRCS := $(shell find $(SRC_DIRS) -name '*.cpp' | grep -v $(MAIN_SRC))
...
# As an example, ./build/hello.ppc turns into ./build/hello.ppc.d
...
# Build step for CPP source (release)
$(RELEASE_DIR)/%.cpp.o: %.cpp
	mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -c $< -o $@ $(RELEASE_FLAGS)

# Build step for CPP source (debug)
$(DEBUG_DIR)/%.cpp.o: %.cpp
	mkdir -p $(dir $@)
